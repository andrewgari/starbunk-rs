...
- Filesystem Hot Reloading: BunkBot utilizes `notify` to watch its configuration directory. Changes to `bots.yml` (e.g. via Kubernetes ConfigMap updates or manual edits) instantly trigger a bot configuration reload without requiring a restart.
