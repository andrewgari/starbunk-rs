...
use crate::shared::llm::{GenerateRequest, LlmMessage, Registry};
use crate::shared::memory::{MemoryService, MemoryServiceImpl, PgStore, Store};
...
impl EventHandler for Handler {
    async fn ready(&self, ctx: Context, ready: Ready) {
        ...
        let _ = self
            .services
            .get_or_init(|| async {
                let llms = match crate::shared::llm::registry_from_env() {
                    Ok(r) => r,
                    Err(e) => {
...
pub async fn run() -> anyhow::Result<()> {
    crate::run_bot("CovaBot", crate::default_intents(), Handler::new()).await
}
